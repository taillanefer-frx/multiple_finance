  ...
  return (
    <BottomSheet open onClose={() => !busy && onClose()} title={goal.name} description={`Prioridade ${priorityLabel[goal.priority]}`}>
      <div className="space-y-5">
        ...
        <Button fullWidth disabled={busy || progress.currentMonthSaved} onClick={() => void onMonthly(goal)}>{progress.currentMonthSaved ? <><Check size={17} /> Valor deste mês guardado</> : <><CalendarCheck2 size={17} /> Marcar valor do mês como guardado</>}</Button>
        ...
        {error && <p className="rounded-2xl bg-red-50 p-3 text-xs text-danger">{error}</p>}
