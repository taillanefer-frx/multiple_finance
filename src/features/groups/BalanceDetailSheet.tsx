...
import { approveExpense, cancelExpense, markExpensePaid, markInstallmentPaid, updateExpenseDetails } from './groupService'
...
export function BalanceDetailSheet({ target, configured, onRefresh, onClose }: BalanceDetailSheetProps) {
  ...
  async function registerPayment(task: () => Promise<{ milestone: 'one_remaining' | 'completed' | null }>) {
    setBusy(true)
    setError(null)
    try {
    ...
    return (
      <BottomSheet open onClose={onClose} title={installment.title} description="Parcelamento ativo">
        <div className="space-y-5">
          ...
          <p className="rounded-2xl bg-canvas p-4 text-sm text-muted">{installment.paidInstallments} pagas · {installment.remainingInstallments} restantes</p>
          ...
          <ReactionBar target={{ kind: 'installment', id: installment.id }} reactions={installment.reactions} disabled={!configured} onReact={react} />
          {configured && installment.remainingInstallments > 0 && <Button fullWidth disabled={busy} onClick={() => void registerPayment(() => markInstallmentPaid(installment.id))}><CheckCircle2 size={16} /> Marcar parcela atual como paga</Button>}
    ...
    <BottomSheet open onClose={() => !busy && onClose()} title={expense.title} description={`${expenseTypeLabel[expense.type]} · ${expenseStatusLabel[expense.status]}`}>
      {editing ? (
        ...
        <div className="space-y-5">
          ...
          {expense.installment && <div className="rounded-2xl bg-canvas p-4"><p className="text-xs font-semibold text-muted">Parcelamento</p><p className="mt-2 text-sm font-semibold text-ink">Parcela {expense.installment.currentInstallment} de {expense.installment.totalInstallments}</p><p className="mt-1 text-xs text-muted">{expense.installment.paidInstallments} pagas · {expense.installment.remainingInstallments} restantes</p></div>}
          <ReactionBar target={{ kind: 'expense', id: expense.id }} reactions={expense.reactions} disabled={!configured} onReact={react} />
          {error && <p className="text-xs text-danger">{error}</p>}
          {configured && expense.status !== 'cancelled' && <div className="grid gap-2 sm:grid-cols-2"><Button variant="secondary" disabled={busy} onClick={() => setEditing(true)}><Pencil size={16} /> Editar</Button>{expense.status === 'review' ? <Button variant="secondary" disabled={busy} onClick={() => run(() => approveExpense(expense.id))}><ShieldCheck size={16} /> Aprovar</Button> : expense.status !== 'paid' ? <Button variant="secondary" disabled={busy} onClick={() => void registerPayment(() => markExpensePaid(expense.id))}><CheckCircle2 size={16} /> Marcar paga</Button> : null}<Button variant="danger" disabled={busy} onClick={() => run(() => cancelExpense(expense.id))}><XCircle size={16} /> Cancelar</Button></div>}
          {celebrationModal}
        </div>
      )}
    </BottomSheet>
